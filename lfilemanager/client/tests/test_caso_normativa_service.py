...
class TestCasoNormativaService:
    ...
    def setup_method(self):
        ...
        self.rol_abogado = Rol.objects.create(
        ...
        self.admin = Usuario.objects.create_user(
            email="admin@x.com", password="pass", nombre="Admin",
    ...
    def test_list_references_returns_only_articles_of_requested_case(self):
        CasoNormativa.objects.create(
            oid_caso=self.caso_lawyer_1, oid_codigo=self.articulo
        )
        CasoNormativa.objects.create(
            oid_caso=self.caso_lawyer_1, oid_codigo=self.articulo_2
        )
        self.client.force_authenticate(user=self.lawyer_1)

        response = self.client.get(
            self.list_url, {"oid_caso": self.caso_lawyer_1.oid_caso}
        )
    ...
    def test_unlink_article_removes_reference(self):
        ...
        self.client.force_authenticate(user=self.lawyer_1)
        detail_url = reverse("caso-normativa-detail", args=[relacion.oid_relacion])  # noqa: E501

        response = self.client.delete(detail_url)

        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not CasoNormativa.objects.filter(
            oid_relacion=relacion.oid_relacion
        ).exists()
    ...
    def test_lawyer_cannot_associate_article_to_foreign_case(self):
        ...
        assert not CasoNormativa.objects.filter(oid_caso=self.caso_lawyer_2).exists()   # noqa: E501

    def test_lawyer_does_not_see_references_of_foreign_case(self):
        CasoNormativa.objects.create(
            oid_caso=self.caso_lawyer_2, oid_codigo=self.articulo
        )
        self.client.force_authenticate(user=self.lawyer_1)

        response = self.client.get(
            self.list_url, {"oid_caso": self.caso_lawyer_2.oid_caso}
        )
    ...
    def test_associate_article_case_not_found_returns_404(self):
        ...
        TC-33: Retorna 404/400 al intentar
        asociar un artículo a un caso inexistente.
        """
        self.client.force_authenticate(user=self.lawyer_1)

        response = self.client.post(
            self.list_url, {"oid_caso": 99999, "oid_codigo": self.articulo.oid_codigo}  # noqa: E501
        )
    ...
    def test_associate_article_invalid_code_returns_validation_error(self):
        ...
        response = self.client.post(
            self.list_url,
            {"oid_caso": self.caso_lawyer_1.oid_caso, "oid_codigo": 99999},
        )
