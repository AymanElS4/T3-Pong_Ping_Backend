...
from client.models import Rol, Usuario, CodigoLegal, Caso, EstadoCaso, TipoCaso, Documento

pytestmark = pytest.mark.django_db


class TestHU16NavigableSideIndexService:
    """Pruebas automatizadas para HU-16: Mostrar un índice lateral navegable para códigos o documentos."""
    ...
    def setup_method(self):
        ...
        self.tipo = TipoCaso.objects.create(nombre='Civil', descripcion='Casos Civiles')

        # Datos de prueba para Códigos Legales
        CodigoLegal.objects.create(nombre_norma='COIP', numero_articulo='144', texto_contenido='Homicidio', vigencia=True)
        CodigoLegal.objects.create(nombre_norma='COIP', numero_articulo='145', texto_contenido='Homicidio culposo', vigencia=True)
        CodigoLegal.objects.create(nombre_norma='Código Civil', numero_articulo='1', texto_contenido='La ley es...', vigencia=True)
    ...
    # TC-25: Equivalence Partitioning / Happy Path - Índice navegable de códigos legales por norma
    def test_get_legal_codes_navigable_index_returns_grouped_structure(self):
        ...
        # Verificar que la norma COIP contenga sus 2 artículos para el índice lateral
        coip_entry = next((item for item in data['indice'] if item['norma'] == 'COIP'), None)
        assert coip_entry is not None
        assert coip_entry['total_articulos'] == 2
        articulos_nums = [art['numero_articulo'] for art in coip_entry['articulos']]
        assert '144' in articulos_nums
        assert '145' in articulos_nums

    # TC-26: Equivalence Partitioning / Happy Path - Índice navegable de documentos agrupados por caso
    def test_get_documents_navigable_index_returns_case_tree(self):
        ...
        # Verificar que el documento de prueba aparezca bajo su caso correspondiente
        keys = list(data['indice_documentos'].keys())
        assert len(keys) == 1
        assert "Caso Herencia Civil" in keys[0]
        assert data['indice_documentos'][keys[0]][0]['nombre_archivo'] == 'Escritura_Publica'

    # TC-27: Boundary Value Analysis - Manejo seguro de índice navegable sin registros en BD
    def test_navigable_index_empty_database_returns_clean_empty_structure(self):
    ...
    # TC-28: Security / Decision Table - Intento de consultar índice lateral sin estar autenticado
